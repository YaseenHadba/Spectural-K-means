...
/*
 * run this function when we get input spk , then runs the Spectral kmeans algorethim and returns the centroids
 */
static PyObject *calcspk(int k , int max_iter, int d, PyObject *all_points, PyObject *init_centroids){
    ...
    KMEANS(k, vectors_matrix,num_of_vectors, initial_centroids);
...
/*
 * api for calckspk function
 */
static PyObject *fitcalcspk_api(PyObject *self,PyObject *args){
    int k, max_iter,d;
    PyObject *all_points;
    PyObject *init_centroids;
    if(!PyArg_ParseTuple(args,"iiiOO",&k,&max_iter,&d,&all_points,&init_centroids));
    return Py_BuildValue("O", calcspk(k,max_iter,d,all_points,init_centroids));
}

/*
 * we use this function we get spk input, we calculate and return T Matrix as described in the spk algorithm 
 */
static PyObject *fitgetT(PyObject *all_points, int k, int numPoints, int d ){
    ...
    all_points_matrix=TMatrix(k,all_points_matrix,numPoints,d);
    if(k==0) k=K;
    if(K==1){
        printf("An Error Has Occurred\n");
        exit(1);
...
/*
 * api for fitgetT function
 */
static PyObject *fitgetT_api(PyObject *self,PyObject *args){
    int k, numPoints,d;
    PyObject *all_points;
    if(!PyArg_ParseTuple(args,"Oiii",&all_points,&k,&numPoints,&d));
    return Py_BuildValue("O", fitgetT(all_points,k,numPoints,d));
}
/*
 *  this function calculate and return the specified matrix that requested from the input via goal 
 */
static PyObject *fit(PyObject *all_points, char*  goal, int numPoints, int d){
    ...
    convert_object_to_matrix(all_points,d,numPoints,all_points_matrix);
    result = resultMatrix(all_points_matrix,goal,numPoints,d);
...
/*
 * api for fit function 
 */
static PyObject *fit_api(PyObject *self,PyObject *args){
    int numPoints, d;
    char *goal;
    PyObject *all_points;
    if(!PyArg_ParseTuple(args,"Osii",&all_points,&goal,&numPoints,&d));
...
static PyMethodDef fit_apiMethods[]={
        ...
        {"fitgetT",(PyCFunction)fitgetT_api, METH_VARARGS,
                        PyDoc_STR("kmeans spk")},
        {"calcspk",(PyCFunction)fitcalcspk_api, METH_VARARGS,
                        PyDoc_STR("kmeans spk")},
        {NULL, NULL, 0, NULL}
...
/*
 *function that converts object to matrix(double**) to use it in C
 */
void convert_object_to_matrix(PyObject *all_points, int d, int numPoints, double **all_points_matrix){
    int i,j;
...
/**
 *function that converts matrix(double**) to object to use it in python
 */
void convert_matrix_to_object(double **matrix, int rows, int cols, PyObject *py_result){
    int i , j;
    PyObject *tmp;
